...
function Posts({ companyLists }) {
  ...
  const delCompany = (e) => {
    urlPath += "/delCompany";
    axios
      .delete(urlPath, { params: { companyId: e.target.value } })
      .then((r) => {
        console.log(r);
      });
  ...
  return (
    <tbody>
      {companyLists.map(
        ({ companyId, name, cate, manager, location }, index) => (
          <tr key={companyId}>
            <td style={{ textAlign: "center" }}>
              {companyLists.length - index}
            </td>
            <td>{name}</td>
            <td>{cate}</td>
            <td>{manager}</td>
            <td>{location}</td>
            <td style={{ textAlign: "center" }}>
              <button onClick={editCompany} value={companyId}>
                수정
              </button>
              &nbsp;&nbsp;
              <button onClick={delCompany} value={companyId}>
                삭제
              </button>
            </td>
          </tr>
